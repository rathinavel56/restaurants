...
class Payment extends AppModel
{
    ...
    public function processPayment($id, $body, $type)
    {
        ...
        if ($body['payment_gateway_id'] == \Constants\PaymentGateways::ZazPay) {
        ...
        } elseif ($body['payment_gateway_id'] == \Constants\PaymentGateways::PayTabREST) {
			$paymentGatewaySettings = PaymentGatewaySetting::with('payment_gateway')->where('payment_gateway_id', \Constants\PaymentGateways::PayTabREST)->get()->toArray();
			$post = array();
			$payTab = array();
			...
			$user = User::with('country','state','city')->where('id', $body['user_id'])->first();
			$user->makeVisible(['email']);
			$user = $user->toArray();
			$post['cc_first_name'] = ($user['first_name'] != "")?$user['first_name']:$user['username'];
            $post['cc_last_name'] = ($user['last_name'] != "")?$user['last_name']:"";
            $post['cc_phone_number'] = "96897676022";
            $post['phone_number'] = "96897676022";
			...
			$post['billing_address'] = "Oman, Muscat";
			$post['city'] = "muscat";
			$post['state'] = "muscat";
            $post['postal_code'] = "130";
            $post['country'] = "OMN";
			
			//Customer's Shipping Address (All fields are mandatory)
			$post['address_shipping'] = "Oman, Muscat";
			$post['city_shipping'] = "muscat";
			$post['state_shipping'] = "muscat";
            $post['postal_code_shipping'] = "130";
            $post['country_shipping'] = "OMN";
			
			//Product Information
			$post['products_per_title'] = substr($body['description'], 0, 50);
			$post['quantity'] = "1";
			$post['unit_price'] = $body['amount'];
            $post['other_charges'] = "0";
            $post['amount'] = $body['amount'];
			$post['discount'] = "0";
			$post['currency'] = CURRENCY_CODE;
			
			 //Invoice Information
			$post['title'] = substr($body['description'], 0, 50);
            $post['msg_lang'] = (isset($body['lang']) && $body['lang'] != "")? $body['lang'] :"en";
            $post['reference_no'] = $body['id'];
			
			//Website Information
			$post['site_url'] = $_server_domain_url;
			$post['return_url'] = $body['success_url'];
			$post['cms_with_version'] = "API USING PHP";
			$post['paypage_info'] = "1";
			$data_response = create_pay_page($post);
			if(!empty($data_response) && $data_response["response_code"] == 4012) {
				Wallet::where('id', $body['id'])->update(['paytab_pay_key' => $data_response["p_id"]]);
				$response = array(
                    'data' => $data_response,
                    'error' => array(
                        'code' => 0,
				...
				$response = array(
                    'error' => array(
                        'code' => 0,
                        'message' => 'Unable to redirect to payment gateway.'
                    )
                );
			}
